    ...
    private function getAttachments(\aportela\DatabaseWrapper\DB $db, ?string $search = null): array
    {
        ...
        $data = $db->query(
            sprintf(
                "
                SELECT
                    ATTACHMENT.id, ATTACHMENT.name, ATTACHMENT.size, ATTACHMENT.sha1_hash AS hash, ATTACHMENT.ctime AS createdAtTimestamp, SA.attachment_id AS shareAttachmentId
                FROM DOCUMENT_ATTACHMENT
