...
const createStorageEntry = <T extends StorageValue>(key: string, defaultValue: T) => ({
  get(): T {
    try {
  ...
  set(value: T) {
    try {
      LocalStorage.set(LOCAL_STORAGE_NAMESPACE + key, value as unknown as StorageValue);
    } catch (error) {
      console.error('Error accessing localStorage:', error);
    }
  },
